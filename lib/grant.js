...
function checkClient (done) {
  var self = this;
  this.model.getClient.call(this, this.client.clientId, this.client.clientSecret,
      function (err, client) {
...
function usePasswordGrant (done) {
  ...
  var self = this;
  return this.model.getUser.call(this, uname, pword, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useRefreshTokenGrant (done) {
  ...
  var self = this;
  this.model.getRefreshToken.call(this, token, function (err, refreshToken) {
    if (err) return done(error('server_error', false, err));
...
function sendResponse (done) {
  ...
  // UPSTREAM to support ?SAML? ?middleware?
